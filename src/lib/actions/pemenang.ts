...
export async function cariPesertaUntukPemenang(angkatanId: number, q: string) {
  await requireUser();
  ...
  const { data } = await supabase
    ...
    .limit(10);
  return (data ?? []).map((r: any) => ({ id: r.id, nama: r.bisakh_peserta?.nama as string }));
}
...
export async function simpanPemenang(input: {
  ...
  const pernahDapatHadiah = (pemenangSebelumnya?.length ?? 0) > 0;

  const { id, ...payload } = input;
  const { error } = id
    ? await supabase.from("pemenang").update({ ...payload, pernah_dapat_hadiah: pernahDapatHadiah }).eq("id", id)
    : await supabase.from("pemenang").insert({ ...payload, pernah_dapat_hadiah: pernahDapatHadiah });
