...
<td className="px-4 py-2 font-medium text-ink">{user.nama ?? "-"}
</td>
<td className="px-4 py-2 text-ink/70">{user.email}</td>
<td className="px-4 py-2">
<select value={role} onChange={(e) => setRole(e.target.value as "mq"
| "superadmin")} className="rounded-md border border-ink/15 px-2 py-1 text-xs">
...
<input type="checkbox" checked={statusAktif} onChange={(e) =>
setStatusAktif(e.target.checked)} />
{statusAktif ? <Badge>Aktif</Badge> : <Badge>Tidak
Aktif</Badge>}
</label>
</td>
<td className="px-4 py-2 text-right">
<Button variant="secondary" className="px-2 py-1 text-xs"
disabled={loading} onClick={simpan}>
