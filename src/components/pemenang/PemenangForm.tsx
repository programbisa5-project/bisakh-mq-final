...
import { cariPesertaUntukPemenang, simpanPemenang } from "@/lib/actions/pemenang";
import { Button } from "@/components/ui/Button";

export function PemenangForm({ kegiatanId, angkatanId }: { kegiatanId: number; angkatanId: number }) {
  const [open, setOpen] = useState(false);
  const [q, setQ] = useState("");
  const [hasil, setHasil] = useState<{ id: number; nama: string }[]>([]);
  const [dipilih, setDipilih] = useState<{ id: number; nama: string } | null>(null);
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
          <div className="w-full max-w-md rounded-lg bg-white p-5">
            <h3 className="font-display text-lg text-ink">Catat pemenang</h3>
            <div className="mt-4 space-y-3">
              ...
              <input type="number" className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Peringkat (opsional)" value={peringkat} onChange={(e) => setPeringkat(e.target.value)} />
              <select className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" value={keputusan} onChange={(e) => setKeputusan(e.target.value)}>
                {["Belum Diputuskan", "Diberikan", "Tidak Diberikan"].map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
              <textarea className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Catatan" value={catatan} onChange={(e) => setCatatan(e.target.value)} />
            </div>
            <div className="mt-4 flex justify-end gap-2">
              <Button variant="ghost" onClick={() => setOpen(false)}>Batal</Button>
              <Button disabled={loading || !dipilih} onClick={submit}>{loading ? "Menyimpan..." : "Simpan"}</Button>
            </div>
          </div>
        </div>
      ) : null}
