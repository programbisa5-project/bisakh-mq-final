...
export function PekanForm({ angkatanId, existing }: { angkatanId: number; existing?: Pekan }) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
          <div className="w-full max-w-md rounded-lg bg-white p-5">
            ...
            <div className="mt-4 space-y-3">
              <input type="number" className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Nomor pekan" value={form.nomor_pekan} onChange={(e) => setForm({ ...form, nomor_pekan: Number(e.target.value) })} />
              <input className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Nama pekan (opsional)" value={form.nama_pekan} onChange={(e) => setForm({ ...form, nama_pekan: e.target.value })} />
              ...
              <select className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
                {["Belum Mulai", "Berlangsung", "Selesai"].map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <Button disabled={loading} onClick={submit}>{loading ? "Menyimpan..." : "Simpan"}</Button>
            </div>
          </div>
        </div>
      ) : null}
