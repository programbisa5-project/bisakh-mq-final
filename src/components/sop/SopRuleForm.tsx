        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
          <div className="w-full max-w-lg rounded-lg bg-white p-5">
            ...
            <div className="mt-4 space-y-3">
              <input className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Kode rule (unik)" value={form.kode_rule} onChange={(e) => setForm({ ...form, kode_rule: e.target.value })} disabled={!!existing} />
              ...
              <input type="number" className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Prioritas (angka, opsional)" value={form.prioritas} onChange={(e) => setForm({ ...form, prioritas: e.target.value })} />
              <textarea rows={5} className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Isi aturan" value={form.isi_rule} onChange={(e) => setForm({ ...form, isi_rule: e.target.value })} />
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <Button disabled={loading || !form.kode_rule || !form.judul || !form.isi_rule} onClick={submit}>{loading ? "Menyimpan..." : "Simpan"}</Button>
            </div>
          </div>
        </div>
      ) : null}
