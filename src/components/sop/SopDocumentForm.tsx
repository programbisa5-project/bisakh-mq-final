...
<h3 className="font-display text-lg text-ink">Tambah dokumen
SOP</h3>
<p className="mt-1 text-xs text-ink/50">Tempelkan link dokumen
yang sudah diunggah (mis. Google Drive).</p>
...
<input className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Deskripsi" value={form.deskripsi}
onChange={(e) => setForm({ ...form, deskripsi: e.target.value })} />
<input className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="URL dokumen" value={form.file_url}
onChange={(e) => setForm({ ...form, file_url: e.target.value })} />
<input className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Nama file (opsional)" value=
{form.nama_file} onChange={(e) => setForm({ ...form, nama_file:
e.target.value })} />
</div>
<div className="mt-4 flex justify-end gap-2">
<Button variant="ghost" onClick={() =>
setOpen(false)}>Batal</Button>
<Button disabled={loading || !form.judul || !form.file_url} onClick=
