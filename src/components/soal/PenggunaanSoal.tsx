...
import { cekKetersediaanSoal, catatPenggunaanSoal } from "@/lib/actions/soal";
...
export function PenggunaanSoal({
  ...
  const [angkatanId, setAngkatanId] = useState(angkatanList[0]?.id ?? 0);
  const [status, setStatus] = useState<"idle" | "checking" | "tersedia" | "embargo">("idle");
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-sm">
      <select
        ...
        value={angkatanId}
        onChange={(e) => { setAngkatanId(Number(e.target.value)); setStatus("idle"); }}
      >
        {angkatanList.map((a) => (
          <option key={a.id} value={a.id}>{a.nama_angkatan}</option>
        ))}
      </select>
      <Button variant="secondary" className="px-2 py-1 text-xs" onClick={cek} disabled={!angkatanId}>
        Cek ketersediaan
      </Button>
      {status === "checking" ? <span className="text-xs text-ink/50">Mengecek...</span> : null}
      {status === "embargo" ? (
        <span className="text-xs text-rose-600">Belum boleh dipakai — belum lewat 10 angkatan sejak terakhir dipakai.</span>
      ) : null}
      {status === "tersedia" ? (
        <>
          <span className="text-xs text-moss-600">Tersedia untuk dipakai.</span>
          <input
          ...
          />
          <Button className="px-2 py-1 text-xs" disabled={loading} onClick={catat}>
