...
if (!canEdit) {
return existing ? (
<a href={existing.link_sertifikat} target="_blank" rel="noreferrer"
className="mt-2 inline-block text-sm text-moss-600 hover:underline">
Buka sertifikat


</a>
):(
<p className="mt-2 text-sm text-ink/50">Link sertifikat belum
tersedia.</p>
);
}
return (
...
{existing ? (
<a href={existing.link_sertifikat} target="_blank" rel="noreferrer"
className="inline-block text-sm text-moss-600 hover:underline">
Buka link saat ini
</a>
) : null}
...
setLoading(true);
try {
await simpanSertifikat({ peserta_kelas_id: pesertaKelasId,
link_sertifikat: link, keterangan });
