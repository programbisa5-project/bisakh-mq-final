        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
          <div className="w-full max-w-lg rounded-lg bg-white p-5">
            ...
            <div className="mt-4 space-y-3">
              <input className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Nama template" value={form.nama_template} onChange={(e) => setForm({ ...form, nama_template: e.target.value })} />
              ...
              <textarea rows={6} className="w-full rounded-md border border-ink/15 px-3 py-2 text-sm" placeholder="Isi template (disimpan persis apa adanya)" value={form.isi_template} onChange={(e) => setForm({ ...form, isi_template: e.target.value })} />
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <Button disabled={loading || !form.nama_template || !form.isi_template} onClick={submit}>{loading ? "Menyimpan..." : "Simpan"}</Button>
            </div>
          </div>
        </div>
      ) : null}
