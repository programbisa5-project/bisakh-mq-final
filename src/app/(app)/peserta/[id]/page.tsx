  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-ink/10 bg-white p-4">
        ...
        <SertifikatForm pesertaKelasId={pesertaKelasId} existing={sertifikat ?? null} canEdit={isSuperadmin} />
      </section>

      <section className="rounded-lg border border-ink/10 bg-white p-4">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-ink/50">Ajukan perubahan</h2>
        <p className="mt-1 text-xs text-ink/50">
          Jalur ini membuat pengajuan di update_queue yang menunggu persetujuan Superadmin — dipakai MQ.
        </p>
        <UpdateQueueForm pesertaKelasId={pesertaKelasId} currentStatus={pk.status_tb} currentNama={master?.nama} />

        {isSuperadmin ? (
          <EditLangsungForm pesertaKelasId={pesertaKelasId} currentStatus={pk.status_tb} currentNama={master?.nama} />
        ) : null}
      </section>
