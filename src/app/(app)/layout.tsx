...
"@/components/layout/SignOutButton";
...
{ href: "/pembuka-muhadharah", label: "Pembuka Muhadharah",
roles: ["mq", "superadmin"] },
{ href: "/sop", label: "SOP", roles: ["mq", "superadmin"] },
{ href: "/update-queue", label: "Update queue", roles: ["mq",
"superadmin"] },
...
<p className="font-display text-lg leading-tight text-ink">BISAkh
MQ</p>
<p className="text-xs text-ink/50">{user.role === "superadmin" ?
"Superadmin" : "Muraqibah"}</p>
</div>
...
<p className="truncate text-xs text-ink/60">{user.mqUser?.nama
?? user.email}</p>
<SignOutButton />
</div>
</aside>
...
<main className="flex-1 p-4 md:p-8">{children}
</main>
</div>
</div>
