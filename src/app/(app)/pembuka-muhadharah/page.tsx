...
import { PembukaForm } from "@/components/pembuka/PembukaForm";

export default async function PembukaMuhadharahPage() {
  const supabase = createClient();
  const { data: rows } = await supabase.from("pembuka_muhadharah").select("*").eq("status_aktif", true).order("kategori");
