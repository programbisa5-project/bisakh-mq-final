  ...
  return (
    <div>
      ...
      <div className="mt-4 overflow-x-auto rounded-lg border border-ink/10 bg-white">
        {error ? (
          <p className="p-4 text-sm text-rose-600">{error.message}</p>
        ) : !rows || rows.length === 0 ? (
            ...
            <tbody>
              {rows.map((q: any) => (
                <tr key={q.id} className="border-t border-ink/5 align-top">
                  ...
                  <td className="px-4 py-2">{q.jenis_perubahan}</td>
                  ...
                  <td className="px-4 py-2"><Badge>{q.status}</Badge></td>
                  {isSuperadmin ? (
                    <td className="px-4 py-2">
                      {q.status === "Menunggu" ? <QueueActions queueId={q.id} /> : null}
                    </td>
                  ) : null}
                </tr>
              ))}
