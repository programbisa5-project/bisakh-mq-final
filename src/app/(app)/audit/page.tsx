...
? (rows ?? []).filter((r: any) =>
(r.peserta_kelas?.bisakh_peserta?.nama ??
"").toLowerCase().includes(searchParams.q!.toLowerCase())
)
: rows ?? [];
return (
<div>
...
<p className="mt-1 text-sm text-ink/60">Catatan perubahan
bersifat permanen — tidak dapat diedit atau dihapus dari sini.</p>
...
<th className="px-4 py-2">Sebelum →
Sesudah</th>
...
{r.jenis_perubahan}</td>
<td className="px-4 py-2 text-xs text-ink/60">
{r.data_sebelum ?
JSON.stringify(r.data_sebelum) : "-"} → {r.data_sesudah
? JSON.stringify(r.data_sesudah) : "-"}
</td>
