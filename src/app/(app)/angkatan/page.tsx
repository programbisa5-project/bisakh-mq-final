...
import { AngkatanForm } from "@/components/angkatan/AngkatanForm";
...
export default async function AngkatanPage() {
  ...
  const { data: rows } = await supabase.from("angkatan").select("*").order("id", { ascending: false });
        ...
        <table className="w-full text-sm">
          <thead className="bg-ink/5 text-left text-xs uppercase tracking-wide text-ink/50">
            <tr>
              ...
              {isSuperadmin ? <th className="px-4 py-2" /> : null}
            </tr>
          </thead>
          <tbody>
            {withCounts.map((a) => (
              <tr key={a.id} className="border-t border-ink/5">
                ...
                <td className="px-4 py-2">{formatTanggal(a.tanggal_mulai)}</td>
                <td className="px-4 py-2">{formatTanggal(a.tanggal_selesai)}</td>
                <td className="px-4 py-2">{a.jumlah_peserta}</td>
                {isSuperadmin ? (
